...
**Conda Helpfulness**
 24. Checking what environments exist: ==`conda env list`== OR `conda info -e`
25. Checking what packages are installed in current env: ==`conda list`==
26. Export environment (as it was made!) to .yml file: ==`conda env export --from-history \> environment.yml`==
27. Create from .yml file: ==`conda env create -f environment.yml`==
28. Remove a broken environment ==`conda remove -n myenv --all`==
29. Add channels: ==`conda config --append channels roebel`==
30. Show available channels ==`conda config --show channels`== 
...
Then also decided to make my own package, for which I used cookiecutter. I found this immensely confusing.
...
My suggestion is to install spyder separately to conda/miniconda. I then changed a bunch of preferences:

62. In Linux, I set interpreter to `/home/gcl1r25/miniconda3/envs/kymatio-env/bin/python3`
63. IPython console \> Graphics \> "Backend=Qt" makes sure plots are opened in a separate window (a Qt widget) and are interactive
64. Enabled Greedy completion in the IPython console too
65. In 'Completion and Linting' set up 'Autoformat on save' and change formatter to 'black', then also turn on Docstring formatting
66. In debugger, add the line `import matplotlib.pyplot as plt` to be automatically called on debugging
67. In Editor, tick all of the options that sound good, e.g. 'Show indent guides', I changed the document template to be:

Also if you press Ctrl + Alt + D after a function declaration, it auto-inserts a docstring!

69. On windows, you can set up in WSL, by installing WSL, installing miniconda on WSL and then following these instructions [https://docs.spyder-ide.org/current/faq.html#install-wsl2](https://docs.spyder-ide.org/current/faq.html#install-wsl2) 
To transfer Spyder
...
- Add directory called .vscode and make a file called settings.json, and add autoFormatOnSave setting
     \> Then From \<[https://www.anaconda.com/docs/getting-started/miniconda/install#linux](https://www.anaconda.com/docs/getting-started/miniconda/install#linux)\>  
\> ```                             
\> ```         
Title:  
